...
async function handleExport(format: string) {
    ...
    if (resultType.value === "image") {
        ...
        if (format === "svg") {
            ...
            // 直接下载 SVG
            const svgFileName = fileName.value.replace(/\.\w+$/, ".svg");
...
<template>
    <div class="flex min-h-screen flex-col bg-gray-50 dark:bg-gray-950">
        <!-- 顶部信息栏 -->
        <ResultHeader v-if="!loading && !error" :result-type-label="resultTypeLabel" :file-name="fileName"
            :original-format="originalFormat" :width="width" :height="height" :file-size-formatted="fileSizeFormatted"
            :is-video="resultType === 'video'" :duration="videoDuration" @close="handleClose"
                ...
                <div class="w-80 shrink-0 space-y-4">
                    ...
                    <div
                        ...
                        <div class="flex gap-3">
                            <svg class="size-5 shrink-0 text-blue-500" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- 视频元数据对话框 -->
        <VideoMetadataDialog :show="showMetadataDialog" :data-url="mediaData || ''" @close="handleCloseDetails" />
    </div>
</template>

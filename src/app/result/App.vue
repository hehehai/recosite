<script setup lang="ts">
  ...
  onMounted(async () => {
    try {
      ...
      const storageData = await browser.storage.session.get(resultId);
      ...
      // 清理 storage 数据
      await browser.storage.session.remove(resultId);
  ...
  async function exportVideo(_format: VideoFormat) {
    // 视频导出功能（预留）
    showNotification("视频导出功能即将推出", "info");
  }
  ...
  async function handleCopyToClipboard() {
    ...
    try {
      if (resultType.value === "image") {
        ...
        // 视频复制（预留）
        showNotification("暂不支持复制视频到剪贴板", "info");
      }
    } catch (err) {

...
import { closeOffscreenDocument } from "@/utils/recording";
 ...
 */
export async function resetRecordingUI(
 ...
 */
async function updateRecordingIcon(isRecording: boolean): Promise<void> {
  try {
    ...
    console.log(
      `[RecordingState] Icon updated to ${isRecording ? "recording" : "idle"} state`
    );
  } catch (error) {

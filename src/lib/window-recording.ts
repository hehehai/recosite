...
export async function startWindowRecording(
  ...
  try {
    console.log("[WindowRecording] Starting window recording with options:", options);
    ...
    const streamId = await new Promise<string>((resolve, reject) => {
      ...
      chrome.desktopCapture.chooseDesktopMedia(["window", "screen"], (selectedStreamId: string) => {
        if (selectedStreamId) {
